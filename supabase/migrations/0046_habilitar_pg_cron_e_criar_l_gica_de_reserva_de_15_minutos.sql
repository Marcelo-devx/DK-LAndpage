-- 1. Habilitar a extensão pg_cron se disponível (necessário para automação no Supabase)
CREATE EXTENSION IF NOT EXISTS pg_cron;

-- 2. Atualizar a função de criação de pedido para DEDUZIR estoque no ato da criação
...
BEGIN
  ...
  -- 1. Verificar estoque de TODOS os itens antes de começar a processar
  ...
  -- 4. Criar o pedido (Status inicial: Aguardando Pagamento)
...
-- 3. Criar a função de limpeza de pedidos expirados
...
BEGIN
  ...
  -- IMPORTANTE: O trigger 'return_order_stock' que você já tem cuidará de devolver o estoque automaticamente!
...
-- 4. Agendar a limpeza para rodar a cada minuto
SELECT cron.schedule('cancel-expired-orders-every-minute', '* * * * *', 'SELECT public.cancel_expired_orders()');
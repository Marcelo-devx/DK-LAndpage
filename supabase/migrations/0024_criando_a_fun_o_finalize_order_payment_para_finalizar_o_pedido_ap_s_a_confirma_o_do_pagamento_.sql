...
BEGIN
  ...
  SELECT user_id, status, total_price INTO current_user_id, v_order_status, v_total_price
  FROM public.orders
  WHERE id = p_order_id;

  IF NOT FOUND OR current_user_id IS NULL THEN
    RAISE EXCEPTION 'Pedido não encontrado ou não pertence ao usuário autenticado.';
  END IF;
  ...
  WHERE uc.user_id = current_user_id
    ...
    AND EXISTS (
        ...
        AND o.total_price = v_total_price -- Simples verificação de preço para garantir que é o pedido correto
    )
  LIMIT 1;
  ...
  SELECT id INTO v_cart_id FROM public.carts WHERE user_id = current_user_id;
  ...
  SET points = points + FLOOR(v_total_price),
      force_pix_on_next_purchase = false -- Remove a restrição de PIX após a compra

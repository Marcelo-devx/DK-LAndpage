CREATE OR REPLACE FUNCTION public.create_pending_order_from_local_cart(
  ...
  payment_method_input text DEFAULT 'credit_card'
)
 RETURNS record
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Define o rótulo bonito para o banco de dados
  IF is_pix THEN
  ...
  LOOP
    ...
    BEGIN
      IF v_item_type = 'product' THEN
        ...
        IF v_variant_id IS NOT NULL THEN
            SELECT pv.price, pv.pix_price, f.name, pv.volume_ml 
            INTO item_price, item_pix_price, item_name, item_stock
            FROM public.product_variants pv 
            LEFT JOIN public.flavors f ON pv.flavor_id = f.id 
            WHERE pv.id = v_variant_id;
            
            SELECT p.name || ' - ' || COALESCE(f.name, '') INTO item_name 
            FROM public.products p, public.flavors f 
            WHERE p.id = v_item_id AND f.id = (SELECT flavor_id FROM public.product_variants WHERE id = v_variant_id);
  ...
  -- 4. Criar o pedido (AGORA INCLUINDO O PAYMENT_METHOD)
  INSERT INTO public.orders (user_id, total_price, shipping_cost, shipping_address, status, coupon_discount, benefits_used, payment_method)
  VALUES (current_user_id, final_price, shipping_cost_input, shipping_address_input, 'Aguardando Pagamento', discount_amount, benefits_input, payment_method_label)
    ...
    BEGIN
      IF v_item_type = 'product' THEN
        ...
        IF v_variant_id IS NOT NULL THEN
            ...
            SELECT p.name || ' - ' || COALESCE(f.name, '') INTO item_name FROM public.products p, public.flavors f WHERE p.id = v_item_id AND f.id = (SELECT flavor_id FROM public.product_variants WHERE id = v_variant_id);
        END IF;
      ELSIF v_item_type = 'promotion' THEN

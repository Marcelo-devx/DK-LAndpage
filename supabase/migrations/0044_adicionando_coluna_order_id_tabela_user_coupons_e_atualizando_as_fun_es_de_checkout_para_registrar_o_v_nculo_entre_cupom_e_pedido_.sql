-- 1. Adiciona a coluna para registrar o pedido onde o cupom foi usado
ALTER TABLE public.user_coupons ADD COLUMN IF NOT EXISTS order_id bigint REFERENCES public.orders(id);

-- 2. Atualiza a função de checkout (carrinho local) para registrar o pedido no cupom
...
BEGIN
  ...
  LOOP
    ...
    BEGIN
      IF v_item_type = 'product' THEN
        ...
        IF item_price IS NULL THEN RAISE EXCEPTION 'Produto não encontrado.'; END IF;
        IF v_variant_id IS NOT NULL THEN
            SELECT pv.price, pv.stock_quantity, f.name, pv.volume_ml INTO item_price, item_stock, v_flavor_name, v_volume_ml
            FROM public.product_variants pv LEFT JOIN public.flavors f ON pv.flavor_id = f.id WHERE pv.id = v_variant_id;
            IF item_price IS NULL THEN RAISE EXCEPTION 'Variação não encontrada.'; END IF;
        END IF;
        IF item_stock < v_quantity THEN RAISE EXCEPTION 'Estoque insuficiente para o produto %.', item_name; END IF;
      ELSIF v_item_type = 'promotion' THEN
        ...
        IF item_price IS NULL THEN RAISE EXCEPTION 'Promoção não encontrada.'; END IF;
        IF item_stock < v_quantity THEN RAISE EXCEPTION 'Estoque insuficiente para a promoção %.', item_name; END IF;
      END IF;
  ...
  -- 4. Vincula o cupom ao pedido e marca como usado
  IF user_coupon_id_input IS NOT NULL THEN
  ...
  LOOP
    ...
    BEGIN
      IF v_item_type = 'product' THEN
        ...
        IF v_variant_id IS NOT NULL THEN
            SELECT pv.price, f.name, pv.volume_ml INTO item_price, v_flavor_name, v_volume_ml
            FROM public.product_variants pv LEFT JOIN public.flavors f ON pv.flavor_id = f.id WHERE pv.id = v_variant_id;

-- 1. Limpeza Radical: Remove todas as assinaturas possíveis dessa função que possam estar causando conflito
DROP FUNCTION IF EXISTS public.create_pending_order_from_local_cart(numeric, jsonb, jsonb);
DROP FUNCTION IF EXISTS public.create_pending_order_from_local_cart(numeric, jsonb, jsonb, bigint);
...
-- 2. Recriação ÚNICA da função correta (com Doação e todos os parâmetros)

DROP FUNCTION IF EXISTS public.create_pending_order_from_local_cart(numeric, jsonb, jsonb, bigint, text);
...
BEGIN
  ...
  LOOP
    ...
    BEGIN
      ...
      -- LÓGICA DE PREÇO: Se for PIX e tiver preço pix cadastrado (>0), usa ele. Senão usa preço normal.
  ...
  LOOP
    ...
    BEGIN
      ...
      -- Definir qual preço foi gravado no histórico do item
      IF is_pix AND item_pix_price IS NOT NULL AND item_pix_price > 0 THEN
        final_item_price := item_pix_price;
      ELSE
        final_item_price := item_price;

-- Correção da função create_pending_order_from_local_cart
...
BEGIN
  ...
  -- Cancela pedidos antigos em "Aguardando Pagamento"
  ...
  final_total_price := final_price + shipping_cost_input + donation_amount_input;
  
  INSERT INTO public.orders (
      user_id, 
      total_price, -- TOTAL COMPLETO (produtos - desconto + frete + doação)
      shipping_cost, 
      shipping_address, 
      status, 
      coupon_discount, 
  ...
  -- Mas o total_price JÁ inclui frete e doação
  final_total_price := final_price + shipping_cost_input + donation_amount_input;

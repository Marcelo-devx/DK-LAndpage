...
BEGIN
  ...
  FOR item_record IN SELECT * FROM jsonb_array_elements(cart_items_input) AS item
  LOOP
    DECLARE
      -- CORREÇÃO: Acessar as chaves diretamente de item_record (que é o alias 'item' do loop)
      v_item_id bigint := (item_record.item->>'itemId')::bigint;
      v_item_type text := item_record.item->>'itemType';
      v_quantity integer := (item_record.item->>'quantity')::integer;
    BEGIN

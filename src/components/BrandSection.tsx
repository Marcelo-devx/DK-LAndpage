            ...
            {validBrands.map((brand) => (
              <CarouselItem key={brand.name} className="pl-4 basis-1/2 sm:basis-1/3 md:basis-1/4 lg:basis-1/6">
                <div 
                  ...
                  className="group relative flex items-center justify-center h-24 bg-white/5 border border-white/10 rounded-2xl cursor-pointer transition-all duration-500 hover:bg-sky-500/10 hover:border-sky-500/50 hover:shadow-[0_0_20px_rgba(14,165,233,0.2)] overflow-hidden"
                >
                  ...
                  {brand.image_url ? (
                    <img 
                      src={brand.image_url} 
                      alt={brand.name} 
                      className="max-h-12 w-auto object-contain brightness-0 invert opacity-60 transition-all duration-500 group-hover:filter-none group-hover:opacity-100 group-hover:scale-110"
                    />
                  ) : (
                    <span className="text-xs font-black uppercase tracking-widest text-slate-400 group-hover:text-sky-400 transition-colors">
                      {brand.name}
                    </span>
                  )}
                </div>
